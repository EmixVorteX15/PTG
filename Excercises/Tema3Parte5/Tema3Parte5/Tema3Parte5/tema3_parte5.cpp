// Ejercicios del tema 3, parte 5 - PTG
...
GLint g_Width = 512;                          // Ancho inicial de la ventana
GLint g_Height = 512;                         // Altura incial de la ventana
GLint depth_texture_size = 512;				// Ancho y alto de la textura con el mapa de profundidad
GLuint pcf_samples = false;
...
GLuint locUniformSimpleShadow, locUniformPCFSamples;
...
void initFBO()
{
	...
	// - asignarle la unidad de textura 0
	glBindTexture(GL_TEXTURE_2D, renderTex);
	glActiveTexture(GL_TEXTURE0);
	// - reserva espacio de memoria para el objeto textura (con tamaño: depth_texture_size x depth_texture_size)
	...
	// - definir los parámetros de la textura proyectiva relativos a tipo de comparación realizado en la consulta (GL_TEXTURE_COMPARE_*)
	...
	//  - anclar la textura en el punto de anclaje GL_DEPTH_ATTACHMENT
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT,GL_TEXTURE_2D, renderTex, 0);
	//  - indicar que no habrán datos de salida (con inform. de color) en el shader de fragmento.
	//Lenum drawBufs[] = { GL_COLOR_ATTACHMENT0 };
...
void drawFBO(glm::vec3 ligthPos)
{
	...
	// Tarea por hacer: activar el FBO
	glBindFramebuffer(GL_FRAMEBUFFER, fboHandle);
	...
	// Tarea por hacer: dibujar la esfera, la tetera y el toroide en el FBO
	if (shadow)
	{
		glCullFace(GL_BACK);
	}
	...
	// Tarea por hacer: desactivar el FBO
	glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
GLboolean init()
{
	...
	loadSource(vertexShaderID, "shaders/tema3_parte5.vert");
	...
	loadSource(fragmentShaderID, "shaders/tema3_parte5.frag");
	...
	locUniformSimpleShadow = glGetUniformLocation(programID, "uSimpleShadow");
	locUniformPCFSamples = glGetUniformLocation(programID, "uPCF_samples");
	
	// Tarea por hacer: Llamar a initFBO para crear el FBO
	initFBO();
...
void display()
{
	...
	struct LightInfo {
	 glm::vec4 lightPos; // S.R. del mundo
	 glm::vec3 intensity;
	};
	LightInfo light = { glm::vec4(3.0f * cos(angle), 3.0f, 3.0f * sin(angle), 1.0f),  // Pos. S.R. mundo
	...
	// Tarea por hacer: Crear el mapa de sombras (llamar a drawFBO para dibujar la escena en el FBO)
	drawFBO(vec3(light.lightPos));
	...
	// Tarea por hacer: inicializar la matriz de escalado/desplazamiento de las coordenadas de textura (B)
	//const glm::mat4 B = glm::mat4(0.5, 0, 0, 0.5, 0, 0.5, 0, 0.5, 0, 0, 0.5, 0.5, 0, 0, 0, 1.0 );
	const glm::mat4 B = glm::mat4(0.5, 0, 0, 0, 0, 0.5, 0, 0, 0, 0, 0.5, 0, 0.5, 0.5, 0.5, 1.0);

	//glm::mat4 S = B * ProjectionLight * ViewLight * mvp;
	glm::mat4 S;

	// Paso al shader de las propiedades de la luz
	mv = View;
	nm = glm::mat3(glm::transpose(glm::inverse(mv)));
	glm::vec4 lpos = mv * light.lightPos;	// Posición de la luz en el S.R. vista
	...
	glUniform1i(locUniformShadowMap, 0);
	glUniform1i(locUniformSimpleShadow, shadow);
	glUniform1i(locUniformPCFSamples, pcf_samples);

	// Tarea por hacer: Calcular la matriz de transformación de coord locales a coordenadas textura (S) y pasarla al shader
