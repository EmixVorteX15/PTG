...
GLboolean specularActivated = false;		 // Indica si parte especular de la luz está activa.
GLboolean luzAmbiente = false;				 // Indica si parte ambiente de la luz está activa.
GLboolean luzAmbienteHemisferio = false;	 // Indica si parte ambiente hemisferio de la luz está activa.
GLboolean luzArmonicosEsferico = false;	     // Indica si parte armonico esferico de la luz está activa.
GLboolean luzImagenes = false;	             // Indica si parte basado en imágenes de la luz está activa.
...
GLuint locUniformSpecularActivated, locUniformLuzAmbiente, locUniformLuzAmbienteHemisferio, locUniformLuzArmonicosEsferico, locUniformLuzImagenes;
GLuint locUniformMred, locUniformMgreen, locUniformMblue;
...
GLint initObj(std::string file)
{
    ...
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, obj_elements.size() * sizeof(GLuint), obj_elements.data(), GL_STATIC_DRAW); // Array de índices
...
GLboolean init()
{
	...
	loadSource(vertexShaderID, "shaders/tema3_parte4.vert");
	...
	loadSource(fragmentShaderID, "shaders/tema3_parte4.frag");
	...
	// Tarea por hacer: descomentar la siguiente linea para dibujar el dragón
	...
	locUniformLuzAmbiente = glGetUniformLocation(programID, "uLuzAmbiente");
	locUniformLuzAmbienteHemisferio = glGetUniformLocation(programID, "uLuzAmbienteHemisferio");
	locUniformLuzArmonicosEsferico = glGetUniformLocation(programID, "uLuzArmonicoEsferico");
	locUniformLuzImagenes = glGetUniformLocation(programID, "uLuzImagenes");

	// Tarea por hacer: localizar el resto de variables uniform.
	locUniformGroundLight = glGetUniformLocation(programID, "uGroundLight");
	locUniformSkyLight = glGetUniformLocation(programID, "uSkyLight");

	locUniformMred = glGetUniformLocation(programID, "Mred");
	locUniformMgreen = glGetUniformLocation(programID, "Mgreen");
	locUniformMblue = glGetUniformLocation(programID, "Mblue");
	

	locUniformDiffuseMap = glGetUniformLocation(programID, "uDiffuseEnvMap");
	locUniformSpecularMap = glGetUniformLocation(programID, "uSpecularEnvMap");
	...
	// Tarea por hacer: hacer lo mismo para el mapa especular y el mapa de irradiación.
	// Irradiación
...
void display()
{
	...
	LightInfo light = { glm::vec3(-1.0, 1.0f, -1.0f),  // Dir. S.R. mundo
	...
	// Grace Cathedral - Luz ambiente
	glm::vec3 ambientLight = glm::vec3(0.64, 0.39, 0.52);

	// Grace Cathedral - Luz de hemisferio (color del cielo y del suelo)
	glm::vec3 groundLight = glm::vec3(0.578125, 0.1953126, 0.0625); // locUniformGroundLight
	glm::vec3 skyLight = glm::vec3(1.0, 0.6745098, 1.0); // locUniformSkyLight

	// Elementos de M (armónicos esféricos) para Grace Cathedral
	...
	// Matriz de Modelo para Chinese_parade_dragon
	...
	glUniform1i(locUniformDiffuseMap, 1);
	glUniform1i(locUniformSpecularMap, 2);

	glUniform1i(locUniformSpecularActivated, GLuint(specularActivated));
	glUniform1i(locUniformLuzAmbiente, GLuint(luzAmbiente));
	glUniform1i(locUniformLuzAmbienteHemisferio, GLuint(luzAmbienteHemisferio));
	glUniform1i(locUniformLuzArmonicosEsferico, GLuint(luzArmonicosEsferico));
	glUniform1i(locUniformLuzImagenes, GLuint(luzImagenes));

	// Tarea por hacer: pasarle los valores al resto de variables uniform.
	// Hemisferio
	glUniform3fv(locUniformGroundLight, 1, &(groundLight.x));
	glUniform3fv(locUniformSkyLight, 1, &(skyLight.x));

	// Armónicos esféricos
	glUniformMatrix4fv(locUniformMred, 1, GL_FALSE, &Mred[0][0]);
	glUniformMatrix4fv(locUniformMgreen, 1, GL_FALSE, &Mgreen[0][0]);
	glUniformMatrix4fv(locUniformMblue, 1, GL_FALSE, &Mblue[0][0]);
	...
	// Tarea por hacer: descomentar el siguiente conjunto de líneas para dibujar el dragón
...
void keyboard(GLubyte key, GLint x, GLint y)
{
	...
	case 'e': case 'E':
		...
		break;
	case 'h': case 'H':
		luzAmbienteHemisferio = !luzAmbienteHemisferio;
		break;
	case 's': case 'S':
		luzArmonicosEsferico = !luzArmonicosEsferico;
		luzImagenes = false;
		break;
	case 'i': case 'I':
		luzImagenes = !luzImagenes;
		luzArmonicosEsferico = false;
		break;
	}
	return;
}
